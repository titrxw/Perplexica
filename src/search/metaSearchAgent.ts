...
class MetaSearchAgent implements MetaSearchAgentType {
  ...
  private async queryDocs(docs: Document[], llm: BaseChatModel, question, optimizationMode: 'speed' | 'balanced' | 'quality',) {
    if (docs.length > 0 && optimizationMode == 'balanced') {
      ...
      let content = docGroups
          .map(
              (_, index) =>
                  `${index + 1}. ${docGroups[index].metadata.title} ${docGroups[index].pageContent}`,
          )
          .join('\n');
